...
HeaderWidget::HeaderWidget(DBHandler *handler, OrderBook *orderBook, QWidget *parent) : QWidget(parent), handler(handler), orderBook(orderBook) {
...
void HeaderWidget::cancelSim() {
    ...
    if (Gan::startGenerate.joinable()) {
        Gan::startGenerate.join();
    }
}
